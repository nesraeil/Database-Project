...
public class BTree<Key extends Comparable<Key>, Value>
{
    ...
    /**
     * Returns an arraylist containing everything in the table that is greater than or equal to the key
     * @param key is the value to check against
     * @return
     */
    @SuppressWarnings("unchecked")
	ArrayList<Row> getGreaterThanOrEquals(Key key) {
    	ArrayList<Entry> allEntries = getOrderedEntries();
    	ArrayList<Row> result = new ArrayList<Row>();
    	for(Entry currentIndex:allEntries) {
    		if(currentIndex.getKey().toString().compareTo(key.toString()) >= 0) {
    			result.addAll((ArrayList<Row>)(currentIndex.getValue()));
    		}
    	}
    	return result;
    }
    ...
    /**
     * Returns an arraylist containing everything in the table that is greater than the key
     * @param key is the value to check against
     * @return
     */    
    @SuppressWarnings("unchecked")
	ArrayList<Row> getGreaterThan(Key key) {
    	ArrayList<Entry> allEntries = getOrderedEntries();
    	ArrayList<Row> result = new ArrayList<Row>();
    	for(Entry currentIndex:allEntries) {
    		if(currentIndex.getKey().toString().compareTo(key.toString()) > 0) {
    ...
    /**
     * Returns an arraylist containing everything in the table that is less than or equal to the key
     * @param key is the value to check against
     * @return
     */
    @SuppressWarnings("unchecked")
	ArrayList<Row> getLessThanOrEquals(Key key) {
    	ArrayList<Entry> allEntries = getOrderedEntries();
    	ArrayList<Row> result = new ArrayList<Row>();
    	for(Entry currentIndex:allEntries) {
    		if(currentIndex.getKey().toString().compareTo(key.toString()) <= 0) {
    ...
    /**
     * Returns an arraylist containing everything in the table that is less than the key
     * @param key is the value to check against
     * @return
     */
    @SuppressWarnings("unchecked")
	ArrayList<Row> getLessThan(Key key) {
    	ArrayList<Entry> allEntries = getOrderedEntries();
    	ArrayList<Row> result = new ArrayList<Row>();
    	for(Entry currentIndex:allEntries) {
    		if(currentIndex.getKey().toString().compareTo(key.toString()) < 0) {
    ...
    /**
     * Returns an arraylist containing everything in the table that is not equal to the key
     * @param key is the value to check against
     * @return
     */
    @SuppressWarnings("unchecked")
	ArrayList<Row> getNotEquals(Key key) {
    	ArrayList<Entry> allEntries = getOrderedEntries();
    	ArrayList<Row> result = new ArrayList<Row>();
    	for(Entry currentIndex:allEntries) {
    		if(currentIndex.getKey().toString().compareTo(key.toString()) != 0) {
    ...
    /**
     * Returns an arraylist containing everything in the table that is equal to the key
     * @param key is the value to check against
     * @return
     */
    @SuppressWarnings("unchecked")
	ArrayList<Row> getEquals(Key key) {
    	ArrayList<Row> result = (ArrayList<Row>)get(key);
    	return result;
    }
    ...
    /**
     * Removes a specified row from the index
     * @param key The value this row's indexed cell
     * @param row The row to delete
     */
    public void deleteSpecificRow(Key key, Row row) {
		@SuppressWarnings("unchecked")
		ArrayList<Row> toDeleteFrom = (ArrayList<Row>)this.get(key);
    	ArrayList<Row> newIndex = new ArrayList<Row>();
    	if(toDeleteFrom != null) {
    		for(Row r:toDeleteFrom) {
    			if (row != r) newIndex.add(r);
    		}
    		toDeleteFrom.clear();
    		toDeleteFrom.addAll(newIndex);
